...
static void show_liquids(CHAR_DATA *ch)
{
    char buf[MAX_STRING_LENGTH];
    char buf1[MAX_STRING_LENGTH];
    int  liq;
...
REDIT(redit_show)
{
    ...
    for (door = 0; door < MAX_DIR; door++) {
        ...
        if ((pexit = pRoom->exit[door])) {
            ...
            sprintf(
                buf, "-%-5s to [%5d] Key: [%5d]", capitalize(dir_name[door]),
...
static bool change_exit(CHAR_DATA *ch, char *argument, int door)
{
    ...
    if (!str_cmp(command, "link")) {
        ...
        /*	pRoom->exit[door]->vnum = value;                Can't set vnum
         * in ROM */
        ...
        /*	pExit->vnum             = ch->in_room->vnum;    Can't set vnum
         * in ROM */
    ...
    if (arg2[0] == '\0') {
        ...
        if (argument[0] == '\0'
            ...
            if ((to_mob = get_char_room(ch, arg2)) != NULL) {
                 ...
                 */
                if ((wear_loc = flag_value(wear_loc_flags, argument))
                    == NO_FLAG) {
...
OEDIT(oedit_ed)
{
    ...
    if (!str_cmp(command, "format")) {
        ...
        ped->description = format_string(ped->description);
