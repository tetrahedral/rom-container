        ...
        for (pRoomIndex = room_index_hash[iHash]; pRoomIndex != NULL;
            ...
            for (door = 0; door <= 5; door++) {
                if ((pexit = pRoomIndex->exit[door]) != NULL
                    ...
                    /*		    sprintf( buf, "Fix_exits: %d:%d -> %d:%d ->
                       %d.", pRoomIndex->vnum, door, to_room->vnum,
                       rev_dir[door], (pexit_rev->u1.to_room == NULL) ? 0 :
                       pexit_rev->u1.to_room->vnum ); bug( buf, 0 );
                     */
                }
            }
        }
...
void free_mem(void *pMem, int sMem)
{
    ...
    *((void **)mem_ptr)  = rgFreeList[iList];
    rgFreeList[iList] = mem_ptr;
 ...
 * Reports a bug.
 */
void bug_area(const char *str, int param, FILE *fp, const char *area_str)
{
    fpArea = fp;
    strcpy(strArea, area_str);
    bug(str, param);
}
...
void reset_room(ROOM_INDEX_DATA *pRoom)
{
    ...
    for (pReset = pRoom->reset_first; pReset != NULL; pReset = pReset->next) {
        ...
        case 'O':
            ...
            if (pRoom->area->nplayer > 0 || pObjIndex->count >= pReset->arg4)
                break;
